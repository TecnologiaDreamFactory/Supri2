...
const { SHEETS, CONDICOES_GERAIS_COLUMNS } = require("../rfq/batch/xlsxSchema");
...
describe("runCompareBatch + XLSX schema", () => {
  it("abas e headers conforme xlsxSchema (Condições Gerais + Inconsistências)", async () => {
    ...
    assert.strictEqual(wb.worksheets[0].name, SHEETS.CONDICOES_GERAIS);
    assert.strictEqual(wb.worksheets[1].name, SHEETS.INCONSISTENCIAS);

    const cg = wb.worksheets[0].getRow(1);
    assert.strictEqual(cg.getCell(1).value, CONDICOES_GERAIS_COLUMNS[0].header);
    assert.strictEqual(cg.getCell(2).value, CONDICOES_GERAIS_COLUMNS[1].header);
  });

  it("formatação: total recalculado com numFmt BRL na aba Condições Gerais", async () => {
    const header = ["Produto", "Qtd", "UN", "Preço Unit", "Total"];
    ...
    const cellTotal = row2.getCell(5);
    assert.ok(cellTotal.numFmt && String(cellTotal.numFmt).includes("R$"));
  });
});
